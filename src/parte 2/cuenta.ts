...
export class Cuenta {
    ...
    depositar(monto: number): void {
        ...
        if (monto < 0) {
            throw new Error("No se pueden depositar valores negativos");
        } 
    ...
    retirar(monto: number): void {
        ...
        if (monto < 0) {
            throw new Error("No se pueden retirar valores negativos");
        }

        if (monto > this.saldo) {
            throw new Error("No se puede retirar mas del dinero disponible")
        }   
